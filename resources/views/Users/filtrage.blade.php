...
<form method="get" action="{{ url('/filtrage') }}" class="max-w-sm mx-auto mt-5">
    ...
    <div class="flex">
        <select name="category" id="categories" class="flex-grow bg-gray-50 border border-gray-300 text-green text-sm rounded-l-lg focus:ring-blue-500 focus:border-blue-500 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
            <option value="" selected>Choose a Category</option>
            @foreach($Categories as $category)
                <option value="{{ $category->id }}">{{ $category->category_name }}</option>
            @endforeach
        </select>
        <button type="submit" class="px-4 py-2 bg-blue-500 text-white rounded-r-lg">Filter</button>
    </div>
</form>
